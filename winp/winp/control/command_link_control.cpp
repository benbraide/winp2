...
winp::utility::error_code winp::control::command_link::set_note(const std::wstring &value, const std::function<void(command_link &, utility::error_code)> &callback){
	return compute_or_post_task_inside_thread_context([=]{
		return pass_return_value_to_callback(callback, *this, set_note_(value));
...
SIZE winp::control::command_link::compute_additional_size_() const{
	if (note_.empty())
...
winp::utility::error_code winp::control::command_link::set_note_(const std::wstring &value){
	note_ = value;
	if (handle_ != nullptr)
